...
<!-- título-->
	...
	<div class="col-sm-3 center">
		...
		@endif
		@if($user->type =="fellow")
		<a href='{{ url("tablero/{$session->module->program->slug}/foros/{$activity->forum->slug}/agregar-pregunta") }}' class="btn gde download">Agregar Pregunta o Tema al foro [<strong>+</strong>]</a>
		@endif
	</div>
	<!-- descripción-->
			...
			@foreach ($forums as $conversation)
				<div class="row forum_list">
					...
					<div class="col-sm-9">
						...
						@endif
						@if($user->type =="fellow")
						<h2><a href='{{ url("tablero/{$session->module->program->slug}/foros/{$activity->forum->slug}/ver-pregunta/$conversation->slug") }}'>{{$conversation->topic}}</a></h2>
						@endif
						<!--fellow data -->
